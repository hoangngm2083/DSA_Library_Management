...
bool TheDocGiaManager::readItem(std::istream& in, TheDocGia& obj) {
   // Đọc lần lượt các trường từ file
    if (!(in >> obj.ma_the)) return false;
    in.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // bỏ phần dư của dòng

    if (!std::getline(in, obj.ho)) return false;
    if (!std::getline(in, obj.ten)) return false;
    if (!(in >> obj.phai)) return false;
    if (!(in >> obj.trang_thai)) return false;
